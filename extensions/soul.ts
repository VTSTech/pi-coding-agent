...
export default function (pi: ExtensionAPI) {
  ...
  debugLog("soul", `[pi-soul] Config: persistence=${piSoulConfig.persistence} autoLoad=${piSoulConfig.autoLoad}`);
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    if (event.reason === "startup") {
      const flagResult = await handleSoulFlag(pi, soulLoader, activeSoulStore, ctx, piSoulConfig);
      if (flagResult !== false) {
        if (flagResult.type === "handled" && flagResult.action === "activated") {
          autoAppliedSoul = flagResult.soul;
        } else if (flagResult.type === "handled" && flagResult.action === "cleared") {
          autoAppliedSoul = null;
        }
        return;
      }
      if (!piSoulConfig.autoLoad) {
        debugLog("soul", "autoLoad is disabled; skipping persisted soul loading");
        return;
      }
    }
    ...
    handler: async (args, ctx) => {
      ...
      if (!args) {
        ...
        if (ctx.hasUI && typeof ctx.ui.select === "function") {
          const pickResult = await handleInteractiveSoulSelect(soulLoader, ctx, pi, activeSoulStore, piSoulConfig, autoAppliedSoul);
      ...
      if (trimmedArgs === "status") {
        if (!autoAppliedSoul) {
          ...
          ctx.ui.notify(`Active soul: **${autoAppliedSoul.displayName}** (level ${autoAppliedSoul.level})`, "info");
        }
        return;
      }
