...
// Active soul persistence lives in shared/soul-config.ts via createActiveSoulStore().
	...
	private parseManifest(data: any, soulDir: string): SoulManifest {
		...
		const hardwareConstraints: HardwareConstraints | undefined =
			data.hardwareConstraints
				? {
						...
						mobility:
							Mobility[data.hardwareConstraints.mobility] ||
							Mobility.STATIONARY,
		...
		const safety: Safety | undefined = data.safety
			? {
					physical: data.safety.physical
						? {
								contact_policy:
									ContactPolicy[data.safety.physical.contactPolicy] ||
		...
		for (const [name, sensorData] of Object.entries(data.sensors || {})) {
			sensors.push({
				...
				resolution:
					typeof sensorData === "object" ? sensorData.resolution : undefined,
				fps: typeof sensorData === "object" ? sensorData.fps : undefined,
				channels:
					typeof sensorData === "object" ? sensorData.channels : undefined,
			});
		}
		...
		return {
			...
			interaction_mode:
				InteractionMode[data.interactionMode] || InteractionMode.TEXT,
			hardware_constraints: hardwareConstraints,
			safety,
			sensors,
			actuators,
...
export default function (pi: ExtensionAPI) {
	...
	// Load piSoul config from settings files (global + project override)
	const piSoulConfig: PiSoulConfig = loadPiSoulConfig();
	debugLog(
		"soul",
		`[pi-soul] Config: persistence=${piSoulConfig.persistence} autoLoad=${piSoulConfig.autoLoad}`,
	);

	// Create active soul store based on config
	const activeSoulStore: ActiveSoulStore = createActiveSoulStore(piSoulConfig);
	debugLog("soul", `[pi-soul] Store: ${activeSoulStore.describe()}`);

	// Register CLI flags
	pi.registerFlag("soul", {
		type: "string",
		description: "Set the soul to load at startup (e.g. --soul dev).",
	});
	pi.registerFlag("soul-level", {
		type: "string",
		description:
			"Set progressive disclosure level for --soul (1-3, default 2).",
	});
	let autoAppliedSoul: {
		name: string;
		displayName: string;
	...
	const makeSessionAccessor = (ctx: ExtensionContext): SessionAccessor => ({
		appendEntry: (type: string, data?: unknown) =>
			// biome-ignore lint/suspicious/noExplicitAny: pi.appendEntry is not typed
			(pi as any).appendEntry(type, data),
		getEntries: () =>
	...
	/**
	 * Apply a soul: set autoAppliedSoul, persist to store, emit activated event, notify.
	 */
	async function applySoul(
		manifest: SoulManifest,
		level: number,
		sessionAccessor: SessionAccessor,
		source: "cli" | "command" | "startup",
		ctx: ExtensionContext,
	): Promise<void> {
		autoAppliedSoul = {
			name: manifest.name,
			displayName: manifest.display_name,
			prompt: soulLoader.buildSystemPrompt(manifest, level),
			level,
		};
		activeSoulStore.save(
			{ active: true, soul: manifest.name, level, updatedAt: Date.now() },
			sessionAccessor,
		);
		pi.events.emit("soul:activated", {
			...
			level,
			manifest,
			persistence: piSoulConfig.persistence,
			autoLoad: piSoulConfig.autoLoad,
			source,
		});
		debugLog("soul", `[pi-soul] Soul activated: ${manifest.display_name}`);
		ctx.ui.notify(`🪷 Soul activated: ${manifest.display_name}`, "info");
		ctx.ui.setStatus("pi-soul", manifest.display_name);
	}

	/**
	 * Clear the active soul: clear store, nullify, emit deactivated, notify.
	 */
	function clearSoul(
		sessionAccessor: SessionAccessor,
		source: "cli" | "command" | "startup",
		...
		const prevSoul = autoAppliedSoul
			? {
					soul: autoAppliedSoul.name,
					displayName: autoAppliedSoul.displayName,
					level: autoAppliedSoul.level,
				}
			: undefined;
		activeSoulStore.clear(sessionAccessor);
		autoAppliedSoul = null;
		pi.events.emit("soul:deactivated", {
			previousSoul: prevSoul?.soul,
			previousDisplayName: prevSoul?.displayName,
			previousLevel: prevSoul?.level,
			persistence: piSoulConfig.persistence,
			autoLoad: piSoulConfig.autoLoad,
			source,
		});
		debugLog("soul", "[pi-soul] Active soul cleared");
	...
	pi.on("session_start", async (event, ctx) => {
		...
		const sessionAccessor = makeSessionAccessor(ctx);

		// For in-process session changes (reload, new, resume, fork),
		// restore active soul from the persisted store regardless of autoLoad.
		// The soul was explicitly activated within this Pi process.
		// Fresh Pi startup (reason === "startup") respects autoLoad — but only
		// when persistence is global. Session/none modes skip auto-load on startup.
		if (event.reason !== "startup") {
			await restoreFromStore(sessionAccessor, ctx);
			return;
		}

		// Handle --soul CLI flag, or fall back to autoLoad
		if (await handleSoulFlagInSession(sessionAccessor, ctx)) return;

		await autoLoadPersistedSoul(event.reason, sessionAccessor, ctx);
	});

	/**
	 * Restore active soul from the persisted store (internal-only, no notify).
	 * Sets autoAppliedSoul and emits soul:activated if a persisted soul is found.
	 */
	async function restoreFromStore(
		sessionAccessor: SessionAccessor,
		ctx: ExtensionContext,
	): Promise<void> {
		const active = activeSoulStore.load(sessionAccessor);
		debugLog(
			"soul",
			`[pi-soul] RestoreFromStore — store has active soul: ${
				active?.soul ?? "none"
			}`,
		);
		if (active && active.active && active.soul) {
			try {
				const manifest = await soulLoader.load(active.soul, active.level || 2);
				// Apply silently — no notifications, no persist (already persisted)
				autoAppliedSoul = {
					name: manifest.name,
					displayName: manifest.display_name,
				...
				pi.events.emit("soul:activated", {
					...
					persistence: piSoulConfig.persistence,
					autoLoad: piSoulConfig.autoLoad,
					/* source reflects the current event context */
				});
				ctx.ui.setStatus("pi-soul", manifest.display_name);
				debugLog(
					"soul",
					`[pi-soul] Restored soul from store: ${manifest.display_name}`,
				);
			} catch (err) {
				debugLog("soul", `[pi-soul] Failed to restore soul from store: ${err}`);
				autoAppliedSoul = null;
				if (ctx.hasUI) {
					ctx.ui.notify(
						`⚠️ Previously active soul "${active.soul}" not found. Use /soul <name> to set one.`,
						"warning",
					);
				}
			}
	...
	/**
	 * Handle --soul and --soul-level CLI flags.
	 * Returns true if a --soul flag was processed (skip autoLoad).
	 */
	async function handleSoulFlagInSession(
		...
		const soulLevelFlag = pi.getFlag("soul-level") as string | undefined;

		if (soulLevelFlag && !soulFlag) {
			debugLog(
				"soul",
				"[pi-soul] --soul-level ignored: no --soul flag provided",
			);
			if (ctx.hasUI) {
				ctx.ui.notify(
					"⚠️ --soul-level ignored: --soul flag is required.",
					"warning",
				);
			}
		}

		if (soulFlag === undefined || String(soulFlag).trim().length === 0) {
			return false;
		}

		const trimmedFlag = String(soulFlag).trim();

		if (isSoulClearValue(trimmedFlag)) {
			clearSoul(sessionAccessor, "cli", ctx);
			debugLog(
				"soul",
				`[pi-soul] Active soul cleared via --soul ${trimmedFlag}`,
			);
			return true;
		}

		// --soul <name>
		const flagLevel = soulLevelFlag
			? Math.max(1, Math.min(3, parseInt(String(soulLevelFlag), 10) || 2))
			: 2;
		try {
			const manifest = await soulLoader.load(trimmedFlag, flagLevel);
			await applySoul(manifest, flagLevel, sessionAccessor, "cli", ctx);
			debugLog(
				"soul",
				`[pi-soul] Soul activated via --soul flag: ${manifest.display_name}`,
			);
		} catch (err) {
			debugLog(
				"soul",
				`[pi-soul] Failed to load soul from --soul flag: ${err}`,
			);
			if (ctx.hasUI) {
				ctx.ui.notify(
					`⚠️ Could not load soul "${trimmedFlag}" from --soul flag: ${err}`,
					"warning",
				);
			}
		}
	...
	/**
	 * Auto-load persisted active soul on fresh startup when using
	 * global persistence with autoLoad enabled. Session/none modes
	 * skip auto-load on fresh startup — restore only on /reload /new.
	 */
	async function autoLoadPersistedSoul(
		eventReason: string,
		sessionAccessor: SessionAccessor,
		ctx: ExtensionContext,
	): Promise<void> {
		if (!piSoulConfig.autoLoad || piSoulConfig.persistence !== "global") {
			debugLog(
				"soul",
				piSoulConfig.autoLoad
					? "[pi-soul] autoLoad only applies to global persistence; skipping"
					: "[pi-soul] autoLoad is disabled; skipping persisted soul loading",
			);
			return;
		}
		...
		const active = activeSoulStore.load(sessionAccessor);
		if (active) {
			debugLog("soul", `[pi-soul] Found persisted active soul: ${active.soul}`);
			try {
				const manifest = await soulLoader.load(active.soul!, active.level || 2);
				await applySoul(
					manifest,
			...
			if (souls.length > 0) {
				...
				if (eventReason === "startup" && ctx.hasUI) {
					ctx.ui.notify(
	...
	pi.registerCommand("soul", {
		description:
			"Use a soul for the current session. Usage: /soul <name> | off | status. Persists per piSoul.persistence config.",
		handler: async (args, ctx) => {
			...
			if (!args) {
				...
				if (choice.startsWith("❌ off")) {
					const sessionAccessor = makeSessionAccessor(ctx);
					clearSoul(sessionAccessor, "command", ctx);
					return;
				}

				if (choice === "———") return; // separator — do nothing

				// Must be a soul name — strip emoji prefix and load
				const soulName = choice.replace(/^🔮 /, "");
				const level = 2;
				try {
					const soul = await soulLoader.load(soulName, level);
					const systemPrompt = soulLoader.buildSystemPrompt(soul, level);
					const sessionAccessor = makeSessionAccessor(ctx);
					await applySoul(soul, level, sessionAccessor, "command", ctx);
					pi.sendMessage(
						{
							customType: "soulspec",
					...
					ctx.ui.notify(
						`Now using soul: ${soul.display_name} (level ${level}). This soul will persist according to your persistence config.`,
						"success",
					);
				} catch (error) {
					if (
						(error as Error).message &&
						(error as Error).message!.includes("Soul not found")
					) {
						const matches = soulLoader.findMatchingSouls(
							new RegExp(soulName, "i"),
						);
						if (matches.length > 0) {
							const matchList = matches.slice(0, 5).join(", ");
							const suggestion =
								matches.length > 5
									...
									: "";
							ctx.ui.notify(
								`No exact match found for "${soulName}". Did you mean one of these?\n\n${matchList}${suggestion}\n\nTry one of these exact names, or use a more specific pattern.`,
								"warning",
							);
						} else {
							const allSouls = soulLoader.getAllSouls();
							if (allSouls.length > 0) {
								...
								const remaining =
									allSouls.length > 10
										...
										: "";
								ctx.ui.notify(
									`No soul found matching "${soulName}".\n\nAvailable souls:\n\n${soulList}${remaining}\n\nUse /souls to see all available souls, or try a partial match like 'dev' or 'assist'.`,
									"warning",
								);
							}
						}
