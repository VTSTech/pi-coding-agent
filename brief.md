...
> Generated: 04-13-2026 | Auditor: Super-Z (GLM-5) | Commit: d46d8bb | Version: 1.1.6
...
| **Total Source** | ~7,500 lines across 12 TypeScript files + 1 JSON theme + 4 test files |
...
| `extensions/status.ts` | 474 | Composable status bar via `ctx.ui.setStatus()` | Major rewrite in 1.1.3–1.1.4, prompt measurement fix in 1.1.6. Uses named slots (not footer overwrite). Detects local vs cloud provider to hide CPU/RAM. 11 event listeners. Pi version via `execSync("pi -v 2>&1")`. `measurePromptFromPayload()` reads system prompt from `before_provider_request` event payload (works with any Pi version). `system-prompt` slot flushed from `flushStatus()` for deterministic ordering. |
| `extensions/api.ts` | 751 | Runtime API mode/URL/thinking/provider switcher | `readSettings()`/`writeSettings()` for settings.json. `/api provider set|list` sub-command for default provider management. Tab completion for modes + providers. |
| `shared/ollama.ts` | 601 | Ollama API helpers, models.json I/O, provider detection | Imported by ALL 8 extensions. TTL cache (2s), `EXTENSION_VERSION` (1.1.5 in code constants), `BUILTIN_PROVIDERS` (11 providers), `detectProvider()`, `fetchContextLengthsBatched()`. |
| `shared/security.ts` | 794 | Command blocklist (66 total, partitioned), SSRF (26 patterns), path validation, security mode toggle, audit log | Imported by 3 extensions + status. Partitioned: 41 CRITICAL (always) + 25 EXTENDED (max mode). Mode-aware SSRF: 19 ALWAYS_BLOCKED + 7 MAX_ONLY. `validatePath()` with `fs.realpathSync()` for symlink bypass prevention. `getSecurityMode()`/`setSecurityMode()` persisted in `security.json`. `AUDIT_LOG_PATH` exported. |
...
| `extensions/security.ts` | 486 | Security tool registrations for Pi's tool execution hooks | Wraps shared security functions. Intercepts `tool_call` and `tool_result` events. `/security mode basic|max` command, `/security-audit` command + `security_audit` tool. Tab completion for mode arguments via `pi.registerCompletion()`. |
...
| `shared/package.json` | — | Shared package manifest (canonical) | Subpath exports only (no barrel), `"type": "module"`. Exports ALL 7 modules: format, ollama, security, types, debug, model-test-utils, react-parser (complete since 1.1.6). NO `"."` entry — do not add one. |
...
| Pi interception | `pi.on("before_provider_request", ...)` for capturing generation params (status.ts uses this for prompt measurement and RespMax display) |
...
| Security model | Blocklist + validation (not sandboxing). 66 blocked commands (41 CRITICAL + 25 EXTENDED), 26 SSRF hostname patterns (19 ALWAYS + 7 MAX_ONLY), path validation, injection detection. Mode-aware: basic/max toggle. Exported check functions from shared. |
| Config persistence | `~/.pi/agent/settings.json` for Pi settings, `~/.pi/agent/models.json` for model config, `~/.pi/agent/security.json` for security mode, `~/.pi/agent/react-mode.json` for ReAct toggle |
...
- **Status bar uses composable slots** — `status.ts` uses `ctx.ui.setStatus("slot-name", value)` with unique slot names (NOT `pi.setFooter()`). All slots are flushed from `flushStatus()` for deterministic ordering. All slots are cleared on `session_shutdown`. If status crashes mid-session, stale slots persist until restart.
...
- **`status.ts` Pi version detection** uses `execSync("pi -v 2>&1", { encoding: "utf-8", timeout: 5000 })` — a raw shell call, inconsistent with the `fetch()` migration pattern used elsewhere. Captures stderr to handle `pi -v` outputting there.
...
- **Security mode defaults to `max`** — if `security.json` doesn't exist, `getSecurityMode()` returns `"max"`. `validatePath()` blocks access to `security.json` via the sensitive paths list, so only direct `fs` calls in shared/security.ts can interact with it.
...
| Prompt measurement | `measurePromptFromPayload()` from `before_provider_request` event | Works with any Pi version since it reads the raw messages array; primary `ctx.getSystemPrompt()` tried first with `debugLog()` on failure |
...
3. **Check Known Landmines** — version bumping (4 places), models.json as source of truth, no barrel exports, registerCommand single-arg completion limitation
