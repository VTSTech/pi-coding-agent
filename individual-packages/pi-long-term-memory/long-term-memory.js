...
module.exports = __toCommonJS(ltm_exports);
...
// node_modules/@vtstech/pi-shared/debug.js
var DEBUG_ENABLED = process?.env?.PI_EXTENSIONS_DEBUG === "1";
function debugLog(module2, message, ...args) {
  if (!DEBUG_ENABLED) return;
  const timestamp = (/* @__PURE__ */ new Date()).toISOString();
  console.debug(`[pi-ext:${module2}] ${timestamp} ${message}`, ...args);
}
...
function getMemoryPath(pi) {
  ...
  return (0, import_path.join)(agentDir, MEMORY_FILE);
}
...
function saveMemory(pi, store) {
  try {
    ...
    (0, import_fs.writeFileSync)(path, JSON.stringify(store, null, 2), "utf8");
  } catch (error) {
...
${list}`, "info");
  ...
  pi.registerTool({
    ...
    parameters: typebox_exports.Object({
      action: typebox_exports.String({ description: "Action: get, add, list, clear, clear-meta, meta" }),
      content: typebox_exports.Optional(typebox_exports.String({ description: "Content for add action" })),
      tags: typebox_exports.Optional(typebox_exports.String({ description: "Comma-separated tags" }))
    }),
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (memoryStore.memories.length > 0) {
      ctx.ui?.notify?.(
        `Long-term memory loaded: ${memoryStore.memories.length} memories available`,
        "info"
      );
    }
  });
  pi.registerTool({
    ...
    parameters: typebox_exports.Object({
      content: typebox_exports.String({ description: "Memory content to store" }),
      tags: typebox_exports.Optional(typebox_exports.String({ description: "Comma-separated tags" })),
      reason: typebox_exports.Optional(typebox_exports.String({ description: "Why this is worth remembering" }))
    }),
    async execute(toolCallId, params, _signal, _onUpdate, ctx) {
      const content = params.content || "";
      const tags = (params.tags || "").split(",").map((t) => t.trim()).filter(Boolean);
