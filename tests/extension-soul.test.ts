...
describe("extensions/soul.ts — extension integration", () => {
	...
	describe("session_start handler", () => {
		it("restores soul from store on reload reason", async () => {
			...
			assert.equal(activated.length, 1, "should emit soul:activated on reload");
			assert.equal(activated[0].payload.soul, "test");
		});
		...
		it("restores soul on new/resume/fork regardless of autoLoad", async () => {
			...
			assert.equal(
				activated.length,
				1,
				"should restore even with autoLoad=false on non-startup",
			);
		});
	...
	describe("/soul command — no args", () => {
		it("shows interactive selector when no args given", async () => {
			...
			// Mock select returns first option (status), so status should show
			assert.ok(ctx.notifyCalls.length > 0);
			// Status says either "No soul is currently active" or shows active soul
			assert.ok(
				ctx.notifyCalls[0].msg.includes("No soul is currently active") ||
					ctx.notifyCalls[0].msg.includes("Active soul:"),
			);
		});
	});
