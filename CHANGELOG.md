...
All notable changes to the Pi Coding Agent Extensions (`@vtstech/pi-coding-agent`) will be documented in this file.
...
- **pi-soul: Configurable persistence and startup soul flag** (`extensions/soul.ts`, `shared/soul-config.ts`)
  - New `soul-config.json` config files (`~/.pi/agent/soul-config.json` / `.pi/soul-config.json`):
    - `persistence`: `"global"` (default) | `"session"` | `"none"` — controls where the active soul is stored
    - `autoLoad`: `true` (default) | `false` — controls whether the persisted soul is auto-applied on startup
...
- **Missing exports in @vtstech/pi-shared package** (`npm-packages/shared/package.json`)
  - Added missing subpath exports: `./config-io`, `./provider-sync`, and `./test-report`.
  - These exports were defined in the package.json exports field but missing from the package, causing "Package subpath is not defined by exports" errors when importing from these modules.
  - Fixed in both `npm-packages/shared/package.json` and `.build-npm/shared/package.json`.
...
- **Missing `await` on async `readModifyWriteModelsJson` calls in api.ts** (`extensions/api.ts`) — [MAINT-02 follow-up]
  - The MAINT-02 refactor replaced direct `readModelsJson()` + `writeModelsJson()` calls with `readModifyWriteModelsJson()`, which is an `async` function returning `Promise<boolean>`. However, the four call sites (`setMode`, `setUrl`, `setThink`, `handleCompat`) called it without `await`, meaning the `if (!written)` guard checked a Promise object (always truthy) instead of the boolean result. This caused esbuild/Pi's bundler to fail parsing the file with `Expected ")" but found "Provider"` at line 300, crashing Pi on startup.
  - Added `async` to the four function declarations and `await` to each `readModifyWriteModelsJson()` call. Verified esbuild bundles cleanly and 341/341 tests pass.
